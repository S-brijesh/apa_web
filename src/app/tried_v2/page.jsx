...
const SerialMonitor = () => {
  ...
  const addTestData = useCallback(() => {
    ...
    if (plotData.length > 0) {
      ...
      variation = Math.max(3, Math.min(10, (baseY * 0.05)));
    }

    // Add test points
    for (let i = 0; i < 10; i++) {
      const x = baseX + Math.floor((Math.random() * variation * 2) - variation);
      const y = baseY + Math.floor((Math.random() * variation * 2) - variation);
  ...
  // Calculate visible domain for y-axis with improved auto-scaling
  const yDomain = useMemo(() => {
    ...
    console.log(`Y-axis range: ${min} to ${max} (range: ${max-min})`);
    return [min, max];
  }, [plotData]);
      ...
      <div className="bg-white p-4 rounded-lg shadow border border-gray-200 mb-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-semibold text-gray-700">
            Real-time {displayMode === 'ecg' ? 'ECG Plot' : displayMode === 'xy' ? 'X-Y Plot' : 'Time Series'}
          </h3>
          ...
          <ResponsiveContainer width="100%" height="100%">
            {displayMode === 'ecg' && (
              <LineChart data={plotData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                ...
                />
                <Tooltip 
                  formatter={(value, name) => [value, name === 'y' ? 'Y Value' : 'Time']}
            ...
            {displayMode === 'xy' && (
              <LineChart data={plotData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                ...
                <XAxis 
                  ...
                  name="X Position" 
                  label={{ value: 'X Value', position: 'insideBottomRight', offset: 0 }}
                  tick={{ fontSize: 12 }}
                  allowDataOverflow={false}
                />
                <YAxis 
            ...
            {displayMode === 'time' && (
              <LineChart data={plotData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                ...
                <YAxis 
                  ...
                  orientation="right"
                  domain={['dataMin - 2', 'dataMax + 2']}
                  label={{ value: 'X Value', angle: 90, position: 'insideRight' }}
                  tick={{ fontSize: 12 }}
                />
                <Tooltip />
                <Line 
